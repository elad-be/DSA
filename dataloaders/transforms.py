...
class Rotate(object):
    ...
    def __call__(self, img):
        ...
        print("rotate")
        return itpl.rotate(img, self.angle, reshape=False, prefilter=False, order=0)#elad edit
...
class Resize(object):
    ...
    def __call__(self, img):
        ...
        #### all this frunction is elad edit
        
        #s1 = int(img.shape[0] * self.size)#elad edit
        #s2 = int(img.shape[1] * self.size)#elad edit
        #new_s = (s1,s2)
        #return img #elad edit
        return np.array(Image.fromarray((img*1).astype(np.uint8)).resize(size = (1,1)).convert('RGB'))#elad edit
        if img.ndim == 3:
            return img
            a = np.array(Image.fromarray((img*1).astype(np.uint8)).resize(size = (1,1)).convert('RGB'))#elad edit
            #return misc.imresize(img, self.size, self.interpolation) 
            print("res")#elad edit
            a = np.array(Image.fromarray((img*1).astype(np.uint8)).resize(size = (1,1)).convert('RGB'))#elad edit
            print(a)#elad edit
            print(a.shape)#elad edit
            ...
            #return [np.array(Image.fromarray(img.astype(np.uint8)).resize(size = self.size))] #elad edit
            print("res")#elad edit
            a = np.array(Image.fromarray((img*1).astype(np.uint8)).resize(size = (1,1)).convert('RGB'))#elad edit
            print(a)#elad edit
            print(a.shape)#elad edit
            return np.array(Image.fromarray((img*1).astype(np.uint8)).resize(size = new_s).convert('RGB')) #elad edit
        else:
...
class ColorJitter(object):
    ...
    def __call__(self, img):
        ...
        pil = Image.fromarray((img*1).astype(np.uint8)).convert('RGB')
